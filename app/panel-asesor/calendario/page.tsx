...
import { ArrowLeft, BellRing, CalendarDays, ChevronLeft, ChevronRight, Clock3, Plus, Trash2, UserRound } from 'lucide-react'
    ...
    <main className="min-h-screen bg-[#F5F2EE] px-4 pb-32 pt-7 text-[#17313A] sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <div className="mb-6 flex items-center justify-between">
          <button onClick={() => router.push('/panel-asesor')} className="group flex items-center gap-2 text-sm font-bold text-[#294952] transition-colors hover:text-[#0F2027]"><span className="grid size-8 place-items-center rounded-full border border-[#B9C3C0] bg-white transition-transform group-hover:-translate-x-0.5"><ArrowLeft className="size-4" /></span>Volver al panel</button>
          <div className="hidden items-center gap-2 text-xs font-bold text-[#294952] sm:flex"><span className="size-2 rounded-full bg-[#2D7A68]" />Agenda sincronizada</div>
        </div>
        ...
        <section className="relative overflow-hidden rounded-[28px] bg-[#17313A] px-6 py-7 text-white shadow-[0_20px_45px_rgba(23,49,58,0.18)] sm:px-8 sm:py-9">
          ...
          <div className="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
            <div className="max-w-2xl"><p className="mb-3 text-xs font-bold uppercase tracking-[0.2em] text-[#F5C8B0]">Agenda Conectia</p><h1 className="font-serif text-4xl font-semibold tracking-tight text-white sm:text-5xl">Tu agenda, con claridad.</h1><p className="mt-3 max-w-xl text-sm font-medium leading-6 text-white sm:text-base">{isAdmin ? 'Vista global para anticiparte a cada movimiento de tu equipo.' : 'Un espacio para tener cada oportunidad, visita y cierre exactamente donde debe estar.'}</p></div>
          ...
          <div className="relative mt-7 grid grid-cols-3 border-t border-white/30 pt-5 sm:max-w-xl"><div><p className="text-2xl font-bold text-white">{todayEvents}</p><p className="mt-0.5 text-xs font-semibold text-white">Para hoy</p></div><div className="border-l border-white/30 pl-5"><p className="text-2xl font-bold text-white">{weekEvents}</p><p className="mt-0.5 text-xs font-semibold text-white">En 7 días</p></div><div className="border-l border-white/30 pl-5"><p className="text-2xl font-bold text-white">{reminderEvents.length}</p><p className="mt-0.5 text-xs font-semibold text-white">Por atender</p></div></div>
        </section>
        ...
        {showForm && <form onSubmit={createEvent} className="mt-6 rounded-[24px] border border-[#D7D5CF] bg-white p-5 shadow-[0_14px_35px_rgba(23,49,58,0.08)] sm:p-6">
          <div className="mb-5 flex items-center justify-between"><div><h2 className="font-serif text-2xl font-semibold">Crear un momento</h2><p className="mt-1 text-sm font-medium text-[#46545A]">Todo lo necesario, sin formularios pesados.</p></div><span className="hidden rounded-full bg-[#EFF5F2] px-3 py-1 text-xs font-bold text-[#185646] sm:block">Nuevo registro</span></div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <p className="self-end pb-2 text-xs font-medium leading-5 text-[#46545A]">Define cuántos meses antes quieres recibir el recordatorio.</p>
          </div>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[390px_minmax(0,1fr)]">
          <aside className="space-y-6">
            <section className="rounded-[24px] border border-[#DDDAD4] bg-white p-5 shadow-[0_12px_30px_rgba(23,49,58,0.06)]">
              ...
              <div className="grid grid-cols-7 gap-y-2 text-center">{WEEK_DAYS.map((day, index) => <span key={`${day}-${index}`} className="pb-1 text-[10px] font-bold text-[#46545A]">{day}</span>)}{calendarDays.map((day, index) => {
