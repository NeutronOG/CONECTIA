...
export function FeaturedPropertiesCarousel() {
  ...
  return (
    <div className="relative w-full flex flex-col md:block md:h-[600px] rounded-[32px] overflow-hidden group shadow-2xl shadow-black/30 bg-[#F6F2EE] dark:bg-[#0F2027]">
      ...
      <div
        className="relative h-[300px] sm:h-[360px] w-full md:absolute md:inset-0 md:h-full bg-cover bg-center transition-all duration-1000 ease-out transform scale-100 md:scale-105 md:group-hover:scale-110"
      ...
      {/* Mobile gradient at bottom of image for smooth card blend */}
      <div className="md:hidden absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-[#F6F2EE] to-transparent pointer-events-none" />

      {/* Content — Split layout */}
      <div className="relative -mt-8 md:mt-0 flex-1 md:h-full flex flex-col md:flex-row z-10">
        {/* Left: Image area (empty on desktop, content visible through overlay) */}
        <div className="hidden md:block md:w-[55%]" />

        {/* Right: Info Panel */}
        <div className="flex-1 md:w-[45%] flex flex-col justify-end md:justify-center px-4 pt-10 pb-6 md:p-10 bg-[#F6F2EE] dark:bg-[#0F2027] md:bg-transparent md:dark:bg-transparent rounded-t-[32px] md:rounded-none">
          ...
          <div className="relative bg-white dark:bg-[#0F2027]/80 md:bg-white/80 md:dark:bg-[#0F2027]/80 backdrop-blur-xl border border-[#17313A]/10 dark:border-white/10 rounded-[28px] p-5 sm:p-6 md:p-8 shadow-xl md:shadow-2xl">
            ...
            <div className="relative space-y-4 md:space-y-5">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] md:text-[11px] text-[#4A4F57] dark:text-[#B0ACA6] font-bold">
      ...
      {/* Dots Navigation */}
      <div className="absolute -bottom-1 md:bottom-5 left-1/2 -translate-x-1/2 flex items-center gap-2 z-20 pb-4 md:pb-0">
