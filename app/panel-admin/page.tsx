...
export default function PanelAdminPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'Exclusiva': return 'bg-[var(--conectia-arcilla)]/10 text-[var(--conectia-arcilla)] border-[var(--conectia-arcilla)]/20'
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: 'rgba(199,143,123,0.15)', border: '1px solid rgba(199,143,123,0.3)' }}>
                <Shield className="w-5 h-5 text-[var(--conectia-arcilla)]" />
              </div>
              <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={async () => { await logout(); router.push('/login') }} className="flex items-center gap-2 px-4 py-2 text-[#B0ACA6] hover:text-white hover:bg-white/5 rounded-xl transition-all text-sm border border-white/10 hover:border-[var(--conectia-arcilla)]/30">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
              {[
                { label: 'Propiedades', value: totalPropiedades, sub: `${propiedadesDisponibles} disp • ${propiedadesExclusivas} exc`, accent: 'var(--conectia-arcilla)', icon: Building2 },
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div className="lg:col-span-2 relative bg-white/[0.03] backdrop-blur-md border border-white/10 rounded-[24px] p-6 overflow-hidden">
                ...
                <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2 relative">
                  <BarChart3 className="w-4 h-4 text-[var(--conectia-arcilla)]" /> Distribución de Propiedades
                </h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 relative">
                  {[
                    { label: 'Disponibles', value: propiedadesDisponibles, total: totalPropiedades, color: '#22c55e' },
                    { label: 'Exclusivas', value: propiedadesExclusivas, total: totalPropiedades, color: 'var(--conectia-arcilla)' },
            ...
            <div className="relative bg-white/[0.03] backdrop-blur-md border border-white/10 rounded-[24px] overflow-hidden">
              <div className="px-6 pt-5 pb-4 border-b border-white/10 flex items-center justify-between">
                <h3 className="text-sm font-bold text-white flex items-center gap-2">
                  <Building2 className="w-4 h-4 text-[var(--conectia-arcilla)]" /> Propiedades Recientes
                </h3>
                <button onClick={() => setActiveTab('properties')} className="text-xs font-bold text-[var(--conectia-arcilla)] hover:text-[var(--conectia-arcilla-soft)] flex items-center gap-1 transition-colors">
              ...
              <div className="p-6">
                {loading ? (
                  <div className="flex items-center justify-center py-8">
                    <div className="w-8 h-8 border-2 border-[var(--conectia-arcilla)]/30 border-t-[var(--conectia-arcilla)] rounded-full animate-spin" />
                  </div>
                  ...
                  <div className="space-y-3">
                    {propiedades.slice(0, 5).map((propiedad) => (
                      <div key={propiedad.id} className="flex items-center gap-4 p-4 bg-white/[0.02] rounded-xl border border-white/5 hover:border-white/15 transition-all group">
                        <div className="w-10 h-10 bg-[var(--conectia-arcilla)]/10 rounded-xl flex items-center justify-center flex-shrink-0 border border-[var(--conectia-arcilla)]/20">
                          <Home className="w-5 h-5 text-[var(--conectia-arcilla)]" />
                        </div>
            ...
            <div className="relative bg-white/[0.03] backdrop-blur-md border border-white/10 rounded-[24px] overflow-hidden">
              <div className="px-6 pt-5 pb-4 border-b border-white/10 flex items-center justify-between">
                <h3 className="text-sm font-bold text-white flex items-center gap-2">
                  <Award className="w-4 h-4 text-[var(--conectia-arcilla)]" /> Equipo de Asesores
                </h3>
                <button onClick={() => setActiveTab('team')} className="text-xs font-bold text-[var(--conectia-arcilla)] hover:text-[var(--conectia-arcilla-soft)] flex items-center gap-1 transition-colors">
                    ...
                    {asesores.slice(0, 6).map((asesor) => {
                      ...
                      return (
                        <div key={asesor.id} className="flex items-center gap-3 p-4 bg-white/[0.02] rounded-xl border border-white/5 hover:border-white/15 transition-all">
                          <div className="w-10 h-10 rounded-xl bg-[var(--conectia-arcilla)]/10 flex items-center justify-center border border-[var(--conectia-arcilla)]/20 flex-shrink-0">
                            <span className="text-xs font-bold text-[var(--conectia-arcilla)]">{getAsesorInitials(asesor.nombre)}</span>
                          </div>
                          <div className="flex-1 min-w-0">
                          ...
                          <div className="text-right shrink-0">
                            <p className="text-lg font-black text-[var(--conectia-arcilla)]">{propiedadesAsesor.length}</p>
        ...
        {activeTab === 'properties' && (
          <div className="relative bg-white/[0.03] backdrop-blur-md border border-white/10 rounded-[24px] overflow-hidden">
            <div className="px-6 pt-5 pb-4 border-b border-white/10">
              <h3 className="text-sm font-bold text-white flex items-center gap-2">
                <Home className="w-4 h-4 text-[var(--conectia-arcilla)]" /> Todas las Propiedades
                <span className="ml-2 text-[10px] font-bold px-2 py-0.5 rounded-full bg-[var(--conectia-arcilla)]/10 text-[var(--conectia-arcilla)] border border-[var(--conectia-arcilla)]/20">{totalPropiedades}</span>
              </h3>
            </div>
            <div className="p-6">
              {loading ? (
        ...
        {activeTab === 'team' && (
          <div className="relative bg-white/[0.03] backdrop-blur-md border border-white/10 rounded-[24px] overflow-hidden">
            <div className="px-6 pt-5 pb-4 border-b border-white/10">
              <h3 className="text-sm font-bold text-white flex items-center gap-2">
                <Users className="w-4 h-4 text-[var(--conectia-arcilla)]" /> Equipo de Asesores
                <span className="ml-2 text-[10px] font-bold px-2 py-0.5 rounded-full bg-[var(--conectia-arcilla)]/10 text-[var(--conectia-arcilla)] border border-[var(--conectia-arcilla)]/20">{totalAsesores}</span>
              </h3>
            </div>
            <div className="p-6">
              {loading ? (
                    ...
                    return (
                      <div key={asesor.id} className="relative bg-white/[0.02] rounded-[20px] p-5 border border-white/5 hover:border-white/15 transition-all group">
                        <div className="flex items-start gap-4 mb-4">
                          <div className="w-12 h-12 rounded-xl bg-[var(--conectia-arcilla)]/10 flex items-center justify-center border border-[var(--conectia-arcilla)]/20 flex-shrink-0">
                            <span className="text-sm font-bold text-[var(--conectia-arcilla)]">{getAsesorInitials(asesor.nombre)}</span>
                          </div>
                          <div className="flex-1 min-w-0">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          ...
                          <div className="text-center p-2 bg-[var(--conectia-arcilla)]/5 rounded-lg border border-[var(--conectia-arcilla)]/10">
                            <p className="text-lg font-black text-[var(--conectia-arcilla)]">{exclusivas}</p>
